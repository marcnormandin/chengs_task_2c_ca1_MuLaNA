 function mltp_compute_best_match_rotation_rect(obj, session)
    ...
    if strcmpi(obj.getArena().shape, 'square')
        [bfo_dist, angleCorrelations, angleCounts] = compute_best_match_rotation(obj, session, 'square');
    elseif strcmpi(obj.getArena().shape, 'rectangle')
        [bfo_dist, angleCorrelations, angleCounts] = compute_best_match_rotation(obj, session, 'rectangle');
    else
        warning('unsupported shape: %s', obj.getArena().shape);
        return
    end
...
function [bfo_dist, angleCorrelations, angleCounts] = compute_best_match_rotation(obj, session, shape)
    ...
    for iT = 1:numTFiles
        ...
        if strcmpi(shape, 'rectangle')
            pmFolder = obj.Config.placemaps.outputFolderShrunk; % rectangles have to use the shrunk placemaps
        elseif strcmpi(shape, 'square')
            pmFolder = obj.Config.placemaps.outputFolder;
        else
            error('Unsupported shape: %s', shape);
        end

        fileNames = dir(fullfile(session.getAnalysisDirectory(), pmFolder, sprintf('%s_*_%s', prefix, obj.Config.placemaps.filenameSuffix)));
        cellPlacemaps = [];
